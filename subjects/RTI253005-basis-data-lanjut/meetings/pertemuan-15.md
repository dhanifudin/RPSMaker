---
course_code: RTI253005
course_name: Basis Data Lanjut
meeting: 15
sub_cpmk: SCPMK1001-02304
assessment_weight: 1.25
---

# Pertemuan 15 - Studi Kasus 4

## Kemampuan Akhir
- SCPMK1001-02304: Mampu mengintegrasikan PostgreSQL dengan aplikasi dan menyelesaikan studi kasus kompleks melalui proyek basis data terintegrasi.

## Materi Pembelajaran
Studi Kasus 4: implementasi full-text search dan pengelolaan data JSONB untuk domain portal berita.

## Bentuk dan Metode Pembelajaran
Modalitas: Blended Learning. Bentuk: Luring/Daring. Metode: Problem-Based Learning, kolaborasi kelompok, dan presentasi.

## Estimasi Waktu
1 x 3 x 50' tatap muka; 1 x 3 x 50' tugas/praktik mandiri.

## Pengalaman Belajar Mahasiswa
Mahasiswa mengimplementasikan full-text search dan pengelolaan data semi-terstruktur JSONB pada domain portal berita, termasuk pembuatan indeks GIN dan query kompleks.

## Penilaian
Bentuk penilaian: Studi kasus 4 (bagian PBL). Mengacu rubrik RTM.

## Indikator Penilaian
Ketepatan implementasi FTS dan JSONB; kualitas query dan indeks.
