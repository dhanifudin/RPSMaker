---
course_code: RTI253005
course_name: Basis Data Lanjut
meeting: 12
sub_cpmk: SCPMK1001-02304
assessment_weight: 1.25
---

# Pertemuan 12 - Studi Kasus 1

## Kemampuan Akhir
- SCPMK1001-02304: Mampu mengintegrasikan PostgreSQL dengan aplikasi dan menyelesaikan studi kasus kompleks melalui proyek basis data terintegrasi.

## Materi Pembelajaran
Studi Kasus 1: analisis kebutuhan dan perancangan skema basis data PostgreSQL untuk domain e-commerce.

## Bentuk dan Metode Pembelajaran
Modalitas: Blended Learning. Bentuk: Luring/Daring. Metode: Problem-Based Learning, kolaborasi kelompok, dan presentasi.

## Estimasi Waktu
1 x 3 x 50' tatap muka; 1 x 3 x 50' tugas/praktik mandiri.

## Pengalaman Belajar Mahasiswa
Mahasiswa menganalisis kebutuhan data domain e-commerce, merancang skema basis data PostgreSQL dengan optimasi indeks, lalu mengimplementasikan dan menguji query yang diperlukan.

## Penilaian
Bentuk penilaian: Studi kasus 1 (bagian PBL). Mengacu rubrik RTM.

## Indikator Penilaian
Ketepatan rancangan skema; kualitas implementasi dan query.
