---
course_code: RTI253004
course_name: Desain dan Pemrograman Web
sub_cpmk: SCPMK0704-02201, SCPMK0209-02202
assessment_form: UTS praktik
---

# UTS Modul Web Dasar

**Bentuk Tugas/Evaluasi:** UTS praktik

**Sub-CPMK:** SCPMK0704-02201, SCPMK0209-02202

## Deskripsi
Mahasiswa menyelesaikan uts modul web dasar sesuai Sub-CPMK dan konteks mata kuliah.

## Metode Pengerjaan
Menganalisis kebutuhan, mengerjakan artefak praktik/proyek, menguji hasil, mendokumentasikan evidence, dan mempresentasikan keputusan bila diminta.

## Bentuk Format Luaran
Laporan, artefak digital, repository/prototype, evidence pengujian, dan/atau bahan presentasi.

## Rubrik Penilaian

| Indikator Penilaian | Bobot (%) | Kriteria 3 (Baik) | Kriteria 2 (Cukup) | Kriteria 1 (Kurang) |
| --- | ---: | --- | --- | --- |
| Kelengkapan modul web dasar | 8 | Modul mencakup halaman, form, validasi, proses PHP, dan alur navigasi sesuai spesifikasi. | Modul utama tersedia, tetapi ada bagian form, validasi, atau navigasi yang belum lengkap. | Modul tidak memenuhi fitur dasar yang diminta. |
| Kerapian kode dan pemisahan concern | 7 | HTML, CSS, JavaScript, dan PHP tertata, mudah dibaca, dan tidak bercampur secara berlebihan. | Kode cukup berjalan, tetapi struktur file atau pemisahan concern belum rapi. | Kode sulit dipelihara atau banyak duplikasi tanpa struktur. |
| Pengujian dan penjelasan hasil | 5 | Kasus uji input normal/invalid dilakukan, output diverifikasi, dan hasil dapat dijelaskan saat praktik. | Pengujian dilakukan terbatas, atau penjelasan hasil belum lengkap. | Tidak ada pengujian bermakna atau tidak mampu menjelaskan hasil modul. |
